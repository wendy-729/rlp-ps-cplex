            ...
            for project in range(42, 43):
                ...
                with open(filename, 'a', newline='') as f:
                    ...
                    md1.minimize(md1.sum(
                        (cost[k] * (2 * (h) - 1)-mu_kt) * y_kth[k, t, h] for h in list(range(1, max_H + 1)) for k in list(range(0, res))
                        for t in d) + md1.sum(req[i][kk] * x_it[i, tt]*mu_kt for kk in k for t in d for i in list(range(1, activities)) for tt
