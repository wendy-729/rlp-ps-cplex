        ...
        for project in range(1, 10):
            for dtime in dtimes:
                ...
                with open(filename, 'a', newline='') as f:
                    ...
                    # 线性化目标函数引入两个辅助变量
                    e_kt = md1.integer_var_dict(kt, name='e')
                    d_kt = md1.integer_var_dict(kt, name='d')
                    z_kt = md1.integer_var_dict(kt, name='z')

                    # 目标函数
                    md1.minimize(md1.sum(
                        cost[kk] * (d_kt[kk,t]+e_kt[kk,t]) for kk in k for t in d))
                    ...
                    for kk in k:
                        for t in range(1, lftn+1):
                            md1.add_constraint(z_kt[kk,t]-z_kt[kk, t-1] ==e_kt[kk,t]-d_kt[kk,t])

                    for kk in k:
                        md1.add_constraint(e_kt[kk,0] ==z_kt[kk,0])
                        md1.add_constraint(e_kt[kk, lftn] == z_kt[kk, lftn])
