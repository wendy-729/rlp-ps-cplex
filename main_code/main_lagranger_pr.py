            ...
            for project in range(42, 43):
                ...
                with open(filename, 'a', newline='') as f:
                    ...
                    md1.minimize(md1.sum(
                        ...
                        list(range(0, res)) for t in d) + md1.sum(mu_pr[j][i] *(md1.sum((t + duration[projPred[j][i]]) * x_it[projPred[j][i], t] for t in list(range(est_s[projPred[j][i]], lst_s[projPred[j][i]] + 1))) \
                        - md1.sum(tt * x_it[j, tt] for tt in list(range(est_s[j], lst_s[j] + 1))) - M * (1 - md1.sum(
                            x_it[j, tt] for tt in list(range(est_s[j], lst_s[j] + 1))))) \
                                                                  for j in list(range(1, activities)) for i in
                                                                 list(range(nrpr[j]))))
