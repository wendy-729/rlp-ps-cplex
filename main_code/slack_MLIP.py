        ...
        for project in range(1,50):
            ...
            for dtime in dtimes:
                ...
                with open(filename, 'a', newline='') as f:
                    ...
                    # 创建模型
                    md1 = Model()
                    # md1.parameters.mip.strategy.startalgorithm = 4
                    # 资源种类
                    ...
                    md1.add_constraints(
                        ...
                        mandatory)
                    # md1.add_constraints(
                    #     md1.sum(x_it[i, t] for t in list(range(est_lst['est'][i], est_lst['lst'][i] + 1))) >=0 for i in
                    #     mandatory)
                    # 终止活动
                    # md1.add_constraints(s[activities-1]<=lftn)
                    ...
                    #     for i in projPred[j]:
                    #         md1.add_constraints(s[i]+duration[i]<=s[j]+M*(1-md1.sum(x_it[j,tt] for tt in
                    #                                            list(range(est_lst['est'][j],est_lst['lst'][j]+1)))))
                    for j in list(range(1, activities)):
                        for i in projPred[j]:
                            md1.add_constraint(
                    ...
                    for i in range(activities - 1):
                        md1.add_constraint(md1.sum((t + duration[i]) * x_it[i, t] \
                                                   for t in list(
                            range(est_lst['est'][i], est_lst['lst'][i] + 1))) <= lftn)
