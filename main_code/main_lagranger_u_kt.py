    ...
    for actNumber in act:
        ...
        for group in range(1, 2):
            ...
            for project in range(42, 43):
                ...
                #     dtime) + '.txt'
                filename = r'C:\Users\ASUS\Desktop\RLP-PS下界_删除优先关系约束'+'\\J' + str(actNumber)+'delete_precedence_lower_'+str(dtime)+'_'+'.txt'
                # 大修路径
                ...
                with open(filename, 'a', newline='') as f:
                    ...
                    md1.minimize(md1.sum(
                        (mu_kt[kk, t] * md1.sum(req[i][kk] * x_it[i, tt] for i in list(range(1, activities)) for tt
                                                in
                                                list(range(max(est_s[i], t - duration[i] + 1), min(t, lst_s[i]) + 1)))
                         for kk in k for t in d)) + \
                    md1.sum(cost[kk] * z_kt[kk, t] * z_kt[kk, t] - mu_kt[kk, t] * z_kt[kk, t] for kk in k for t in d))
                    # md1.minimize(md1.sum((cost[kk]*z_kt[kk, t]-mu_kt)*z_kt[kk, t] for kk in k for t in d) + md1.sum(req[i][kk] * x_it[i, tt] * mu_kt for kk in k for t in d for i in list(range(1, activities)) for
                    #     tt in list(range(max(est_s[i], t - duration[i] + 1), min(t, lst_s[i]) + 1))))
                    ...
                    for i in range(len(act_time)):
                        ...
                        vl_schedule.append(act_time[i][1])
