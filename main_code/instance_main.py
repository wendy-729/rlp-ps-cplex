    ...
    for project in range(1, 2):
        ...
        for dtime in dtimes:
            ...
            with open(filename, 'a', newline='') as f:
                file = r'C:\Users\ASUS\Desktop\示例\projectData.txt'
                # 初始化数据
                ...
                for i in range(0, len(su)):
                    ...
                    proSu.append(temp1)
                # print('紧后活动集合', proSu)
                datafile = r'C:\Users\ASUS\Desktop\示例\J'
                # 必须执行的活动
                fp_mandatory = datafile + str(actNumber) + '\\mandatory\\' + str(project) + '.txt'

                # mandatory = initfile(fp_mandatory)
                # mandatory_structure = [0, 1, 2, 3, 5, 6, 8, 10]
                # mandatory_structure = [0, 1, 2, 3, 5, 6, 9, 10]
                mandatory_structure = [0, 1, 2, 4, 7, 10]

                mandatory =mandatory_structure
                # 可选集合
                fp_choice = datafile + str(actNumber) + '\\choice\\' + str(project) + '.txt'
                choice = initChoice(fp_choice)
                choice = np.array(choice)

                # 所有可选活动
                fp_choiceList = datafile + str(actNumber) + '\\choiceList\\' + str(project) + '.txt'
                choiceList = initfile(fp_choiceList)

                # 依赖活动
                fp_depend = datafile + str(actNumber) + '\\dependent\\' + str(project) + '.txt'
